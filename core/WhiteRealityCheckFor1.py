...
def bootstrap(ser):
    ...
    '''
    #threshold = 2* np.percentile(mb, [percentile])[0]
    if ave > threshold:
        print("Reject Null Hypothesis = The population distribution of rule returns has an expected value of zero or less (because p_value is small enough)")
    else:
        print("Do not reject Null Hypothesis = The population distribution of rule returns has an expected value of zero or less (because p_value is not small enough)")
    '''
